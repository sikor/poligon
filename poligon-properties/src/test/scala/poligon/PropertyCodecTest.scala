...
import poligon.polyproperty.PropertyObserver.{PropertyObservers, TaskRunner}
...
case class Case2(s: String, instants: Seq[Long]) extends IntOrString
...
class PropertyCodecTest extends FunSuite {
  ...
  val m = FajnyModel(
    ...
    2.3,
    MalyModel(12),
    Case2("aaa", Vector(Instant.now().getEpochSecond, Instant.now().plusSeconds(10).getEpochSecond)),
  ...
  test("update") {
    ...
    val newValue = FajnyModel(
      ...
      Case2("aaa2", Vector(Instant.now().getEpochSecond, Instant.now().plusSeconds(10).getEpochSecond)))
  ...
  val taskRunner = new TaskRunner(monix.execution.Scheduler.Implicits.global, f => println(f.getMessage))
  test("sorted map update") {
    implicit val po: PropertyObservers = PropertyObserver.createRoot(taskRunner)
